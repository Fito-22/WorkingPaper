...
def preprocessor_pred(text, model_type:str ='word2vec') -> (np.ndarray):
    ...
    text = ' '.join(text.split()[:num_of_words_to_keep])
    ...
    if model_type == 'word2vec':
        ...
        text_embedded = embed_paper(word2vec_transfer, data)
        X_pad = text_embedded[:max_length_of_padding]
        return X_pad
